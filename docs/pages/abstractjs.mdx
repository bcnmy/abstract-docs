...
user friendly apps which leverage the latest innovations in blockchain execution, including [chain abstraction](),
[gas sponsorships](), [managing smart account modules]() and much more...

Modern onchain app teams work in a multi-chain environment, where they have to interact with multiple chains, bridges and
intent solvers. `AbstractJS` makes navigating this complex multi-chain hybrid execution environment a breeze, with a rich
library of **multichain utilities**. These include functions like:
- [Getting a unified multichain balance](), 
- [Handling smart account deployments across multiple chains](), 
- [Executing intents](), 
- [Effortessly building complex multi-chain sequences](), 
- [Handling token deployements across multiple chains]()

Built on top of the industry-standard `viem` SDK, AbstractJS enables developes to seamlessly interact with:

- Biconomy [Superbundler](/mee/mee) and the [Modular Execution Environment (MEE)]()
- ERC4337 [Bundler & Paymaster infrastructure](/overview)
...
This is a high level overview of the features of `AbstractJS`. To dive straight into coding with the SDK, check our 
quickstart guides:

- [Building a Chain Abstracted Application](/mee/tutorials/build-chainabstracted-app)
- [Executing Intents](/mee/tutorials/execute-intent)
- [Sponsoring a Transaction with Paymasters](/tutorials/gasless)
- [Cross-Chain Gas Abstraction](/mee/tutorials/pay-gas-across-chains)
- [Working With Account Modules](/modules/validators)
...
both [Biconomy Nexus]() account as well as other smart account implementations. [Click here]() for a list of supported
providers.
...
[AA Quickstart](./quickstart). Not sure where to start? Read our [Choosing Infrastructure Guide](./mee/mee)
:::
```ts
