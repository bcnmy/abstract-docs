# EIP-7702 Gasless Transactions with Privy and Biconomy

In this guide, we demonstrate using Biconomy's AbstractJS together with Privy to enable your users to send gasless (sponsored) transactions using EIP-7702.
...
Now you can use the MEE client to send a gasless transaction:
...
console.log("Gasless transaction submitted:", hash);
```
...
## 10. Send a cross-chain gasless transaction
...
// Execute gasless cross-chain transaction
const { hash: crossChainHash } = await meeClient.execute({
  authorization: txAuthorization,
  feeToken: null, // Gasless
  instructions: crossChainInstructions,
  delegate: true
});

console.log("Cross-chain transaction submitted:", crossChainHash);
```
...
You've now learned how to use Biconomy's AbstractJS with Privy to enable EIP-7702 gasless transactions. This implementation provides:

- Email-based login with automatic wallet creation
- EIP-7702 delegation to create Smart Accounts at the same address
- Fully gasless (sponsored) transactions
...
3. Setting `delegate: true` and `feeToken: null` in the `execute` call