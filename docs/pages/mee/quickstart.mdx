...
The Biconomy MEE SDK revolutionizes how we think about cross-chain operations. 
...
By leveraging Modular Smart Accounts and the 

### Use Case 
For example, you might have

- `300 USDC` on Optimism,
- `400 USDC` on Polygon
- `300 USDC` on Base

but want to:
- supply `1000 USDC` to _Aave_ on Arbitrum.

### Before MEE
Traditionally, this would require multiple manual bridge operations, waiting for confirmations, and 
then interacting with Aave. 

The user would have to sign multiple transactions, switch networks in-between those 
transactions and would need to have native or ERC20 tokens available on all chains 
to pay for gas.

### Modular Execution
With AbstractJS and MEE, this becomes a single operation. 
...
:::info[Open-source and vendor neutral]
MEE has no vendor lock-in - your funds are bridged through existing infrastructure (e.g. LiFi, Across, ...) and you
...
:::danger[🚨 Important]
Your funds must be on the Smart Contract Account (SCA), not your EOA. 
Many users make the mistake of sending funds to their EOA - this will not work with the MEE Node. 

---

If you want to enable MEE Features for EOAs, read about the Biconomy [Fusion Module](/mee/fusion)
:::
...
#### Set Up a `MultichainSmartAccount`

AbstractJS comes with a built-in utility function for easier deployment and
management of Smart Accounts across multiple chains. To learn more about
`MultichainSmartAccount` click [here](/mee/concepts/multichain-smart-account)
```typescript
...
// Get account address.
// When running this example, you must fund this 
// address with either native coins or ERC20 tokens.
mcNexus.deploymentOn(optimism.id).address
```

#### Initialize MEE Client

In order to use the MEE Node, we need to establish a connection to it. This
is done with `createMeeClient`
...
#### Initialize Multi-Chain Contracts

AbstractJS comes built-in with a data structure making it easy to work
with smart contracts deployed across multiple chains. Read about 
`MultichainContract` [here](/mee/concepts/multichain-contract)
...
:::tip[Using AbstractJS Exported Token Contracts]

To remove the need for developers to search for contract addresses 
across multiple chains, we have exported common ERC20 tokens in
AbstractJS

---
```ts
// Import the USDC Multichain Contract object 
...
#### Build and Execute the Supertransaction
