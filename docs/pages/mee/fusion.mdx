...
EIP7702 exection and regular EOA execution. This means that apps using the Fusion module will get:
...
The trigger transaction is a transaciton which will be posted onchain and
...
const { hash } = await meeClient.executeSignedFusionQuote({
  signedFusionQuote: await meeClient.signFusionQuote({
...
After the `ERC20` token arrives to the Companion account, Tthe MEE Node then calls the `execute` function
